...
void ShaderRegexGroup::link_command_lists_and_filter_index(UINT64 shader_hash)
{
	...
	if (command_list.commands.empty() && post_command_list.commands.empty() && filter_indices.empty())
		return;
	...
	// Set the filter indices for partner filtering. A ShaderRegex can add
	// indices without replacing an explicit ShaderOverride filter_index.
	for (float index : filter_indices)
		shader_override->add_filter_index(index);
...
bool unlink_shader_regex_command_lists_and_filter_index(UINT64 shader_hash)
{
	...
	if (shader_override->filter_index != shader_override->backup_filter_index ||
	    shader_override->filter_indices != shader_override->backup_filter_indices) {
